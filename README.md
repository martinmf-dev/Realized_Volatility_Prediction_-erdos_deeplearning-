...
## The training loop 
Our custom training loop comes with a scheduler option that updates training rate according to validation loss plateau, and an early stopping option which reloads the best weight dictionary with the best validation loss. 

In practice, we would apply the model with the best weight dictionary received from the training loop and fine tuning on the pre-segregated test set for a truly fair model comparison. 
