...
def Default_Ext_Trans():
    #Getting raw data
    get_raw=input("Would you like to download raw data from kaggle (expected to be done in 240 seconds)?(Y/N)\n").upper()
    if get_raw=="Y": 
    ...
    #Recoverying time
    recover_time=input("Would you like to recover time id order (expected to be done in 60 seconds)?(Y/N)\n").upper()
    if recover_time=="Y": 
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Time id order recovered in {elapsed_time} seconds. Starting to save it. \n")
    ...
    create_data=input("Would you like to transform raw data into ready-to-use data?(Y/N)\n").upper()
    if create_data=="Y": 
        print("Transforming data...\n")
        
        create_RV=input("Create RV by row id (expected to be done in 60 seconds)?(Y/N)\n").upper()
        ...
        create_trade=input("Create trade data (expected to be done in 20 seconds)?(Y/N)\n").upper()
        ...
        if create_trade=="Y":
            ...
            #Create embedding id
            unique_ids = sorted(df_total['stock_id'].unique())
            id_to_emb = {stock_id: i for i, stock_id in enumerate(unique_ids)}
            df_total['emb_id'] = df_total['stock_id'].map(id_to_emb)
            end_time = time.time()
            elapsed_time = end_time - start_time
        ...
        create_ts=input("Create timesereis data (expected to be done in 360 seconds)?(Y/N)\n").upper()
